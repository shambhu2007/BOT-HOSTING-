# -*- coding: utf-8 -*-
#𝐍𝐨𝐯𝐚 𝐇𝐨𝐬𝐭𝐢𝐧𝐠 — Credit: @lII_KING_IIl
import telebot
import subprocess
import os
import zipfile
...
BOT_NAME = "𝐍𝐨𝐯𝐚 𝐇𝐨𝐬𝐭𝐢𝐧𝐠"
CREDIT = "@lII_KING_IIl"

# Welcome banner image (fixed image sent to every new user)
WELCOME_IMAGE_URL = 'https://t.me/podting7473/81'
...
def md_escape(text):
    ...
    (names, usernames, filenames) so it can't break parse_mode='Markdown' messages."""
    if text is None:
        return ''
    text = str(text)
    for ch in ['_', '*', '`', '[']:
...
def run_script(script_path, owner_id, user_folder, file_name, msg_obj, attempt=1):
    ...
    try:
        ...
        try:
            ...
            bot.reply_to(msg_obj, f"✅ `{md_escape(file_name)}` started! (PID: {process.pid})", parse_mode='Markdown')
        except Exception as e:
...
def create_admin_panel():
    ...
    markup.row(types.InlineKeyboardButton('🔙 Back', callback_data='back_to_reply_keyboard'))
    return markup
...
def process_send_command(message, script_key):
    ...
    try:
        ...
        if proc and proc.poll() is None:
            ...
            proc.stdin.flush()
            bot.reply_to(message, f"✅ Sent to `{md_escape(info['file_name'])}`:\n`{md_escape(message.text)}`", parse_mode='Markdown')
        else:
            bot.reply_to(message, f"❌ `{md_escape(info['file_name'])}` is not running.", parse_mode='Markdown')
    except Exception as e:
...
def _logic_send_welcome(message):
    ...
    welcome_text = (
        f"💀 *{BOT_NAME}*\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"👋 Hello, {md_escape(name)}!\n\n"
        f"🆔 ID: `{user_id}`\n"
        f"👤 Username: `@{md_escape(username) or 'Not set'}`\n"
...
def handle_document(message):
    ...
    try:
        try:
            ...
            bot.send_message(OWNER_ID,
                f"📁 File `{md_escape(fname)}` from {md_escape(message.from_user.first_name)} (`{user_id}`)",
                parse_mode='Markdown')
        except Exception as e:
            logger.warning(f"Could not forward to owner: {e}")

        wait = bot.reply_to(message, f"⏳ Downloading `{md_escape(fname)}`...", parse_mode='Markdown')
...
def handle_callbacks(call):
    ...
    if data == 'verify_join':
        if is_member(user_id):
            ...
            call.message.from_user = call.from_user   # use the real clicking user, not the bot
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_reply_keyboard','speed','stats']:
    ...
    try:
        ...
        elif data == 'back_to_reply_keyboard': back_to_reply_keyboard_callback(call)
...
def check_files_callback(call):
    ...
    if not files:
        ...
        try:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Back", callback_data='back_to_reply_keyboard'))
...
def back_to_reply_keyboard_callback(call):
    bot.answer_callback_query(call.id)
    try:
    ...
    bot.send_message(call.message.chat.id, "🏠 Main Menu",
                     reply_markup=create_reply_keyboard(call.from_user.id))
